...
NCC.datepicker.monthLayout = function (_id, _param) {
  _id = _id || "";
  _param = _param || {};
  var _obj = {
    cols: [
      {
        view: "button", type: "icon", icon: "wxi-angle-left", width: 60, click: function (id, event) {
          NCC.datepicker.addMonth(_id, -1);
        }, tooltip: "前月へ"
      },
      { view: "ndatepicker", id: _id, type: "month", format: "%Y年%m月", value: new Date(), width: 200 },
      {
        view: "button", type: "icon", icon: "wxi-angle-right", width: 60, click: function (id, event) {
          NCC.datepicker.addMonth(_id, 1);
        }, tooltip: "翌月へ"
      },
      {}
